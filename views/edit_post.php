...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden mb-6">
        ...
        <!-- Tab content -->
        <div x-data="{ content: '<?php echo str_replace("'", "\\'", htmlspecialchars($post['content'])); ?>', title: '<?php echo str_replace("'", "\\'", htmlspecialchars($post['title'])); ?>', previewTitle: '<?php echo str_replace("'", "\\'", htmlspecialchars($post['title'])); ?>', previewContent: '<?php echo str_replace("'", "\\'", htmlspecialchars($post['content'])); ?>' }" x-init="$watch('content', value => previewContent = value); $watch('title', value => previewTitle = value);">
            <!-- Edit tab -->
            <div x-show="activeTab === 'write'" class="p-6">
                <form action="edit_post.php?id=<?php echo $post['id']; ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="flex justify-end pt-4">
                        <a href="post.php?id=<?php echo $post['id']; ?>" class="mr-4 px-6 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition duration-150">
                            Annuleren
                        </a>
...
`inline code`</code></pre>
            ...
            <div x-show="activeTab === 'preview'" class="p-6">
                ...
                <div class="prose prose-lg max-w-none" x-html="previewContent ? parseMarkdown(previewContent) : '<p class=\'text-gray-500 italic\'>Typ content in de Bewerken tab om een voorbeeld te zien...</p>'"></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    // Eenvoudige client-side markdown parser
